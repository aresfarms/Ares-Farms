...
export default function FounderChangeReviewPage() {
  ...
  return <main style={{maxWidth:1100,margin:"0 auto",padding:"32px 20px",fontFamily:"system-ui"}}>
    ...
    {snapshot ? <>
      <section><h2>Status</h2><p><strong>Report:</strong> {snapshot.report?.status ?? "No frozen report"}</p><p><strong>Founder authority:</strong> {snapshot.founderReleaseDecision?.status ?? "Not yet available"}</p><p><strong>Activation performed:</strong> No</p></section>
      {snapshot.report ? <>
        ...
        <section><h2>Current blockers</h2>{snapshot.report.blockers.length ? <ul>{snapshot.report.blockers.map((x:string)=><li key={x}>{x}</li>)}</ul> : <p>None in the frozen report.</p>}</section>
      </> : null}
