...
export const DIVESTIBLE_UNITS: DivestibleUnit[] = [
  {
    ...
    services: [
      "postgres (dedicated source-intelligence schema)",
      "object storage (scraped evidence)",
    ],
  ...
  },
  {
    ...
    sharedDependencies: [GOVERNANCE_CLIENT, SHARED_REGISTRY],
    services: [
      "postgres (dedicated financing-intelligence schema)",
      "payment connector(s)",
    ],
    config: [
      "DATABASE_URL",
      "payment provider keys",
      "provider-license registry endpoint",
    ],
  },
  {
  ...
  },
  {
    ...
    ownedDataStores: ["review-transitions"],
    sharedDependencies: [GOVERNANCE_CLIENT, SHARED_REGISTRY],
...
export function buildCarveOutManifest(unitId: string) {
  ...
  return {
    ...
    dataExport: {
      ...
      portability:
        "Vol III §3.9 — complete export of the unit's own data, no cross-store reads.",
    },
    runAlone: {
      services: u.services,
      config: u.config,
