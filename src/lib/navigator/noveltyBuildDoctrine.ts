...
const SEXUAL_RE: RegExp[] = [
  /\b(?:sex|porn|strip)\s*(?:club|theater|motel|resort|shop)\b/i,
  /\bbrothel\b/i,
  /\badult\s+(?:novelty|entertainment|theme)\b.{0,30}\b(?:building|structure|park|attraction|venue)\b/i,
...
const CODE_EVASION_RE: RegExp[] = [
  ...
  /\bhide\s+(?:it|the\s+\w+)?\s*from\s+the\s+(?:county|city|inspector|code\s+enforcement)\b/i,
