...
const REQUESTED_REVISION = process.env.P5_REPORT_REVISION?.trim() || null;
...
function resolveTargetRevision(): string {
 if (REQUESTED_REVISION) return REQUESTED_REVISION;
 const dir=path.join(process.cwd(),"artifacts","deployments","staging");
 if (!existsSync(dir)) throw new Error("No staging deployment manifest directory exists; set P5_REPORT_REVISION explicitly.");
 for (const name of readdirSync(dir).filter(x=>x.endsWith(".json")&&!x.endsWith("-gates.json")).sort().reverse()) {
  try {
   const record=JSON.parse(readFileSync(path.join(dir,name),"utf8")) as {cloudRunRevision?:unknown};
   if (typeof record.cloudRunRevision==="string"&&record.cloudRunRevision.trim()) return record.cloudRunRevision.trim();
  } catch { /* skip non-manifest evidence */ }
 }
 throw new Error("No staging deployment manifest contains cloudRunRevision; set P5_REPORT_REVISION explicitly.");
}
function main(): void {
 ...
 const targetRevision=resolveTargetRevision();
 ...
 add("target revision is live",liveRevision===targetRevision,`${liveRevision} expected ${targetRevision}`); add("live image is digest pinned",liveImage.includes("@sha256:"),liveImage);
 ...
 const failed=checks.filter(x=>!x.pass); const generatedAtUtc=new Date().toISOString(); const report={schemaVersion:"p5-official-report-authority-readiness-v1",environment:"staging",targetRevision,targetImage:liveImage,blockerId:"P5-B06",blockerStatus:failed.length?"OPEN":"EVIDENCE_READY_HUMAN_APPROVAL_REQUIRED",outcome:failed.length?"FAIL":"PASS",productionAuthorized:false,officialPublicationPermitted:false,publicVerificationPermitted:false,officialReliancePermitted:false,inventoryVersion,inventory,authorization,checks,generatedAtUtc};
