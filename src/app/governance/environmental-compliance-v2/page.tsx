...
type ApiResponse = {
  ok: boolean;
  error?: string;
  v2Result?: EnvironmentalComplianceV2Result;
};
...
function signalToTone(
  status: EnvironmentalComplianceV2Signal["status"]
...
function SignalCard(props: { signal: EnvironmentalComplianceV2Signal }) {
  const { signal } = props;
  return (
...
function ConflictCard(props: {
  conflict: EnvironmentalComplianceV2CrossSourceConflict;
}) {
  return (
    <div
      style={{
...
export default function EnvironmentalComplianceV2Page() {
  const [reviewerRole, setReviewerRole] = useState(
    "Qualified Governance Reviewer"
  );
  const [declaredCustomerTypes, setDeclaredCustomerTypes] = useState(
    "beginning farmer, environmental market participant"
  );
  ...
  const [feeDisclosedBeforeInitiation, setFeeDisclosedBeforeInitiation] =
    useState(true);
  const [spokeIsolationConfirmed, setSpokeIsolationConfirmed] = useState(true);
  const [bankerSpokeIsolated, setBankerSpokeIsolated] = useState(true);
  ...
  const localInput = useMemo<EnvironmentalComplianceV2Input>(
    () => ({
      reviewerRole,
  ...
  const previewResult = useMemo(
    () => composeEnvironmentalComplianceV2(localInput),
    [localInput]
  );
  ...
  async function runComposition() {
    ...
    try {
      const response = await fetch(
        "/api/governance/environmental-compliance-v2",
        {
          method: "POST",
  ...
  return (
    <div style={shellStyle}>
      <div style={containerStyle}>
          ...
          >
            <div>
              <div style={{ fontWeight: 700, fontSize: 20 }}>
                Environmental Compliance v2
              </div>
              <div style={{ ...mutedText, marginTop: 4 }}>
                Internal advisory environmental compliance composition over
                Environmental Intake v2 (which composes the full canonical v2
                stack) plus an advisory replay of the v1
                environmental-compliance gate semantics (no DB writes).
                Operational guidance and review routing only — no external
                environmental provider engagement, fee authorization,
                official environmental report, environmental clearance, NEPA
                determination, Phase I ESA report, or permit issued.
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              ...
              <StatusBadge tone="neutral" label="Fee autonomy" />
            </div>
          </div>
        </header>
          ...
          >
            ...
            >
              <input
                ...
                onChange={(e) => setProviderLicenseVerified(e.target.checked)}
              />
            ...
            >
              <input
                type="checkbox"
                checked={feeDisclosedBeforeInitiation}
                onChange={(e) =>
                  setFeeDisclosedBeforeInitiation(e.target.checked)
                }
              />
              Fee disclosed before initiation
            </label>
            <label
              style={{
                display: "flex",
            ...
            >
              <input
                type="checkbox"
                checked={spokeIsolationConfirmed}
                onChange={(e) => setSpokeIsolationConfirmed(e.target.checked)}
              />
              Environmental Engineering Spoke isolation confirmed
            </label>
            <label
              style={{
                display: "flex",
            ...
            >
              <input
                type="checkbox"
                checked={bankerSpokeIsolated}
                onChange={(e) => setBankerSpokeIsolated(e.target.checked)}
              />
              Banker Spoke isolated
            </label>
            <label
              style={{
                display: "flex",
        ...
        <section style={{ ...panelStyle, padding: 18 }}>
          ...
          >
            {[
              ...
              ["v1 gates", result.summary.v1GateCount],
              ["v1 blockers", result.summary.v1GatesBlockedCount],
              ["v1 triggered", String(result.summary.v1EnvironmentalAssessmentTriggered)],
              ["v1 advancement", String(result.summary.v1LoanPathwayAdvancementAllowed)],
              ["v1 outcome", result.summary.v1AssessmentOutcome],
              ["v1 fee", result.summary.v1FeeAmount],
              ["v1 market rate", result.summary.v1StandardMarketRateAmount],
              [
        ...
        <section style={{ ...panelStyle, padding: 18 }}>
          <div style={{ fontWeight: 600, fontSize: 16, marginBottom: 12 }}>
            v2 governed environmental compliance signals
          </div>
        ...
        <section style={{ ...panelStyle, padding: 18 }}>
          ...
          <div style={{ ...mutedText, fontSize: 13 }}>
            ...
            {result.gateSnapshot.assessmentRequirementStatus} · assessment
            type {result.gateSnapshot.assessmentType} · provider type{" "}
            {result.gateSnapshot.providerType} · outcome{" "}
            {result.gateSnapshot.assessmentOutcome} · blockers{" "}
            {result.gateSnapshot.blockerReasons.length}
          </div>
          {result.gateSnapshot.blockerReasons.length > 0 && (
            <ul style={{ marginLeft: 16, ...mutedText, fontSize: 12 }}>
              {result.gateSnapshot.blockerReasons.map((blocker) => (
