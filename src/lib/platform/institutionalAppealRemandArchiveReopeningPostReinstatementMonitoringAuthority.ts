import type { InstitutionalAppealRemandArchiveReopeningReinstatementEvaluation } from "@/lib/platform/institutionalAppealRemandArchiveReopeningReinstatementAuthority";
...
export const INSTITUTIONAL_APPEAL_REMAND_ARCHIVE_REOPENING_POST_REINSTATEMENT_MONITORING_SCHEMA_VERSION = "institutional-appeal-remand-archive-reopening-post-reinstatement-monitoring-v1";
export type AppealRemandPostReinstatementSignalSeverity = "INFO" | "WARNING" | "MATERIAL" | "CRITICAL";
export type AppealRemandPostReinstatementMonitoringDecision = "CONTINUE" | "REVIEW_REQUIRED" | "RESUSPEND" | "ESCALATE";
export type AppealRemandPostReinstatementMonitoringState = "ACTIVE" | "REVIEW_PENDING" | "RESUSPENSION_REQUIRED" | "ESCALATED" | "COMPLETE";

export type InstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoringPolicy = Readonly<{
...
export type InstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoringSignal = Readonly<{
...
export type InstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoringEvaluation = Readonly<{
  schemaVersion:typeof INSTITUTIONAL_APPEAL_REMAND_ARCHIVE_REOPENING_POST_REINSTATEMENT_MONITORING_SCHEMA_VERSION;
  reinstatementPolicyId:string; monitoringPolicyId:string; canonicalObjectId:string;
  decision:AppealRemandPostReinstatementMonitoringDecision; resultingState:AppealRemandPostReinstatementMonitoringState;
...
export function createInstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoringPolicy(input:InstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoringPolicy){
...
export function createInstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoringSignal(input:InstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoringSignal){
...
export function evaluateInstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoring(input:{reinstatement:InstitutionalAppealRemandArchiveReopeningReinstatementEvaluation;policy:InstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoringPolicy;signals:readonly InstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoringSignal[];humanReviewRefs?:readonly string[];resuspensionNoticeRef?:string|null;evidenceRefs:readonly string[];evaluatedAt:string;auditRefs:readonly string[];replayRef:string;monitoringComplete?:boolean;}):InstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoringEvaluation{
  iso(input.evaluatedAt,"evaluatedAt");const p=createInstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoringPolicy(input.policy);const signals=input.signals.map(createInstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoringSignal);const reviews=list(input.humanReviewRefs??[],"humanReviewRefs");const evidence=list(input.evidenceRefs,"evidenceRefs");const reasons:string[]=[];
  ...
  const hard=reasons.some(r=>r.startsWith("reinstatement:")||r==="matter-not-reinstated"||r==="replay-continuity-required"||r.startsWith("scope-not-monitored:")||r.startsWith("missing-evidence:"));let decision:AppealRemandPostReinstatementMonitoringDecision="CONTINUE";if(hard||(critical&&p.criticalSignalForcesEscalation))decision="ESCALATE";else if(resuspensionTriggered&&!reasons.includes("resuspension-notice-required"))decision="RESUSPEND";else if(aggregateScore>=p.reviewThreshold||reasons.length)decision="REVIEW_REQUIRED";const resultingState:AppealRemandPostReinstatementMonitoringState=input.monitoringComplete&&decision==="CONTINUE"?"COMPLETE":decision==="ESCALATE"?"ESCALATED":decision==="RESUSPEND"?"RESUSPENSION_REQUIRED":decision==="REVIEW_REQUIRED"?"REVIEW_PENDING":"ACTIVE";
  return Object.freeze({schemaVersion:INSTITUTIONAL_APPEAL_REMAND_ARCHIVE_REOPENING_POST_REINSTATEMENT_MONITORING_SCHEMA_VERSION,reinstatementPolicyId:input.reinstatement.reinstatementPolicyId,monitoringPolicyId:p.policyId,canonicalObjectId:input.reinstatement.canonicalObjectId,decision,resultingState,aggregateScore,reasons:Object.freeze([...new Set(reasons)].sort()),affectedScopes:list(signals.map(s=>s.scope),"affectedScopes"),signalRefs:list(signals.map(s=>s.signalId),"signalRefs"),reviewRefs:reviews,evidenceRefs:evidence,evaluatedAt:input.evaluatedAt,auditRefs:list(input.auditRefs,"auditRefs",true),replayRef:ne(input.replayRef,"replayRef")});
}

export const institutionalAppealRemandArchiveReopeningPostReinstatementMonitoringAuthority=Object.freeze({createPolicy:createInstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoringPolicy,createSignal:createInstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoringSignal,evaluate:evaluateInstitutionalAppealRemandArchiveReopeningPostReinstatementMonitoring});
