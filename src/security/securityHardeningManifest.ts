...
export const SECURITY_CONTROLS: SecurityControl[] = [
  // A — account/identity
  { id: "A-mfa-privileged", group: "A", name: "Passkeys/MFA required for owner/operators/admins/reviewers/stewards", status: "implemented", blockingForAlpha: false, blockingForProduction: true, evidence: "src/lib/auth/webauthnMfa.ts + src/lib/auth/accessSecurityRuntime.ts + src/lib/auth/mfaAssurance.ts; passkey enrollment/verification and privileged MFA runtime implemented; staging deployment pending" },
  { id: "A-no-sms-mfa", group: "A", name: "No SMS-only MFA for privileged users", status: "implemented", blockingForAlpha: false, blockingForProduction: true, evidence: "src/lib/auth/mfaAssurance.ts + src/app/security/mfa/page.tsx; MFA assurance method is passkey-only and SMS-only MFA is prohibited; staging deployment pending" },
  ...
  { id: "A-session-revoke", group: "A", name: "Global/per-user session revocation", status: "implemented", blockingForAlpha: false, blockingForProduction: true, evidence: "src/lib/auth/accessSecurityRuntime.ts + src/db/schema/accessSecurity.ts; sessionVersion invalidates prior sessions and lifecycle flows revoke passkeys; staging deployment pending" },
  ...
  { id: "B-perm-multiparty", group: "B", name: "Privileged permission changes require role-separated multi-party approval", status: "implemented", blockingForAlpha: true, blockingForProduction: true, evidence: "securityGovernanceVerification.requireMultiParty · T-multiparty test" },
  ...
  // C — role-separated human governance
  { id: "C-no-single-compromise", group: "C", name: "No single governance principal can compromise the institution", status: "implemented", blockingForAlpha: true, blockingForProduction: true, evidence: "securityGovernanceVerification quorum invariants · T-governance" },
  ...
  { id: "C-no-disable-controls", group: "C", name: "No single governance principal clears protected control-change gates", status: "implemented", blockingForAlpha: true, blockingForProduction: true, evidence: "CRITICAL_ACTIONS owner+independent-review quorum · T-governance" },
  ...
  { id: "C-independent-financial-review", group: "C", name: "Financial high-risk actions require independent review", status: "implemented", blockingForAlpha: true, blockingForProduction: true, evidence: "FINANCIAL_HIGH_RISK requires owner + independent reviewer · T-steward" },
  ...
  { id: "F-plaid-encrypt-at-rest", group: "F", name: "Plaid consumer data + access-token application-layer encryption at rest", status: "implemented", blockingForAlpha: false, blockingForProduction: true, evidence: "plaidSecureRecords ciphertext-only schema + AES-256-GCM envelope store + Secret Manager master key + verify:plaid-at-rest-encryption" },
  ...
  { id: "I-image-scan", group: "I", name: "Container/image scanning + provenance", status: "partial", blockingForAlpha: false, blockingForProduction: true, evidence: "CI builds core/scanner images and fails Trivy on HIGH/CRITICAL; non-root runtime checks active. Cryptographic image signing/provenance attestation remains pending." },
  { id: "I-endpoint-vuln-scan", group: "I", name: "Employee/contractor endpoint vulnerability + patch posture scanning", status: "partial", blockingForAlpha: false, blockingForProduction: true, evidence: "scheduled macOS scanner active on Caitlin endpoint; organization-wide coverage requires enrollment of every in-scope employee/contractor device" },
  { id: "I-rotation", group: "I", name: "Secret rotation procedure", status: "partial", blockingForAlpha: false, blockingForProduction: true, evidence: "risk-based rotation workflow, Secret Manager versions, activation/validation/rollback/retirement evidence implemented; provider-specific automation and current-production attestations remain incomplete" },
  ...
  { id: "O-domain-multiparty", group: "O", name: "Domain transfer / registrar / DNS-authority / ownership change requires owner + independent reviewer", status: "implemented", blockingForAlpha: true, blockingForProduction: true, evidence: "requireMultiParty domain-* (owner + independent reviewer) in src/security/securityGovernanceVerification.ts · verify:domain-governance" },
  ...
  { id: "O-domain-transfer-lock", group: "O", name: "Transfer/registrar lock enabled on every domain", status: "partial", blockingForAlpha: false, blockingForProduction: true, evidence: "manual governance attestation pending (registrar settings not assumed) · DOMAIN_TRANSFER_LOCK_REQUIRED" },
  { id: "O-domain-autorenew", group: "O", name: "Auto-renew enabled on every domain", status: "partial", blockingForAlpha: false, blockingForProduction: true, evidence: "manual governance attestation pending · DOMAIN_AUTORENEW_REQUIRED" },
  { id: "O-domain-dns-review", group: "O", name: "Role-separated DNS security review recorded", status: "missing", blockingForAlpha: false, blockingForProduction: true, evidence: "no role-separated DNS review recorded yet · DOMAIN_DNS_REVIEW_REQUIRED" },
  { id: "O-domain-registrar-mfa", group: "O", name: "Registrar accounts use MFA/passkeys", status: "partial", blockingForAlpha: false, blockingForProduction: true, evidence: "manual governance attestation pending (treat registrar account as critical infrastructure)" },
  { id: "O-domain-continuity", group: "O", name: "Domains in owner/governance continuity + disaster recovery records", status: "doctrine-only", blockingForAlpha: false, blockingForProduction: true, evidence: "docs/security/domain-security-governance.md continuity section; governance emergency package item" },
  ...
  { id: "P-sec-dr", group: "P", name: "SEC-DR-001 — DR aggregate gate (framework + RECOVERY-CERT-001 mandatory sub-gate)", status: "missing", blockingForAlpha: false, blockingForProduction: true, evidence: "AGGREGATE (reviewer decision 2026-06-11): SEC-DR-001 cannot verify unless the recovery certification is valid, unexpired, simulation-backed (RPO/RTO met), AND human-reviewed. RECOVERY-CERT-001 deliberately folded, not a sixth blocker. Currently: recoveryCertificationValid()=false" },
