...
async function main(){process.env.FURLONG_RUNTIME_STATE_DIR=fs.mkdtempSync(path.join(os.tmpdir(),"furlong-recompute-"));
 ...
 const calls:string[]=[]; const deps=[{sourceId:"parcel-tax-authority",sourceVersion:"tax-v2"},{sourceId:"well-permit-authority",sourceVersion:"well-v2"}]; const make=(kind:string)=>()=>{calls.push(kind);return {artifactHash:"a".repeat(64),dependencies:deps,generatedAt:"2026-07-25T16:05:00Z",productionEvidence:true as const}}; const handlers:any={"tax-scenario":make("tax-scenario"),"top-three":make("top-three"),"qualification-result":make("qualification-result"),"property-report":make("property-report")};
