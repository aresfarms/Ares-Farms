...
type AdvancedIntelligenceV2Request = AdvancedIntelligenceV2Input;

function createAdvancedIntelligenceV2TraceId(): string {
  return `advanced-intelligence-v2-${Date.now()}-${Math.random()
    .toString(36)
    .slice(2, 10)}`;
}

export async function POST(req: NextRequest) {
  const traceId = createAdvancedIntelligenceV2TraceId();

  try {
    const body = (await req
      .json()
      .catch(() => ({}))) as AdvancedIntelligenceV2Request;
    const actorId = body.userId ?? body.reviewerRole ?? null;

    const runtimeGuard = runRuntimeGuard({
      operation: "governance.advanced.intelligence.v2.compose",
      module: "api.governance.advanced-intelligence-v2",
      traceId,
      schemaVersion: "advanced-intelligence-v2-request-v0.1.0",
      governanceVersion: "master-volumes-runtime-v0.1.0",
      classificationLevel: "RESTRICTED",
      replayRef: traceId,
      actorId,
      metadata: {
        route: "/api/governance/advanced-intelligence-v2",
    ...
    if (!runtimeGuard.allowed) {
      const observability = createObservabilityEvent({
        eventType: "ADVANCED_INTELLIGENCE_V2_RUNTIME_BLOCKED",
        domain: "runtime",
        severity: "WARN",
        message:
          "Advanced Intelligence v2 runtime guard blocked the request.",
        traceId,
        replayRef: traceId,
        actorId,
        module: "api.governance.advanced-intelligence-v2",
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "Runtime governance guard blocked Advanced Intelligence v2 request.",
          governance: {
            traceId,
            runtimeGuard,
            observability,
    ...
    const versionRuntime = evaluateVersionRuntime({
      ...
      versions: [
        createRuntimeVersionRef(
          "schema",
          "advanced-intelligence-v2-request-v0.1.0",
    ...
    const classifiedInput = classifyRecord(body as Record<string, unknown>, {
      ...
      classificationSource:
        "api-governance-advanced-intelligence-v2-route",
      ...
      consentRequirements: [
        "governance-advanced-intelligence-v2-review-consent",
      ],
    });

    const v2Result = composeAdvancedIntelligenceV2(body);
    ...
    const classifiedOutput = classifyRecord(
      ...
      },
      {
        ...
        sensitivityScope: "governance",
        classificationSource:
          "api-governance-advanced-intelligence-v2-route-output",
        classificationVersion: "classification-runtime-v0.1.0",
        replayRef: traceId,
        disclosureAudience: [
          "authorized-operator",
    ...
    const explanation = createExplanationLineage({
      ...
      outputType: "advanced_intelligence_v2_pack",
      audience: "governance",
      claimType: "recommendation",
      summary:
        "Advanced Intelligence v2 pack composed as advisory, replay-safe, audit-safe, conflict-preserving internal evidence only. No approval, autonomous customer eligibility determination, autonomous pathway determination, autonomous opportunity determination, autonomous intelligence determination, credit decision, lender commitment, public verification, regulatory reliance, tax-credit allocation, environmental clearance, carbon-credit issuance, source certainty claim, or legal reliance is created.",
      ruleVersion: ADVANCED_INTELLIGENCE_V2_RUNTIME_VERSION,
      overlayRefs: [],
      confidenceScore: Math.min(
        0.85,
        Math.max(0.45, 0.45 + v2Result.summary.totalInsightCount / 60)
      ),
      humanReviewRequired: true,
      replayRefs: [traceId],
      auditEventRefs: [],
      metadata: {
        v2DomainCount: v2Result.summary.v2DomainCount,
        v1DomainCount: v2Result.summary.v1DomainCount,
        totalInsightCount: v2Result.summary.totalInsightCount,
        conflictCount: v2Result.summary.conflictCount,
    ...
    const observability = createObservabilityEvent({
      eventType: "ADVANCED_INTELLIGENCE_V2_COMPOSED",
      domain: "operations",
      severity: "INFO",
      message:
        "Advanced Intelligence v2 pack composed through governed runtime controls.",
      traceId,
      replayRef: traceId,
      actorId,
      module: "api.governance.advanced-intelligence-v2",
    ...
    const evidence = await persistGovernanceEvidence({
      ...
      classifications: [
        {
          resourceType: "advanced_intelligence_v2_input",
          resourceId: traceId,
        ...
        {
          resourceType: "advanced_intelligence_v2_output",
          resourceId: traceId,
      ...
      replayVerification: {
        traceId,
        replayRef: traceId,
        targetType: "advanced_intelligence_v2_pack",
        targetId: body.applicationId ?? traceId,
        verificationStatus: versionRuntime.ok ? "PASS" : "WARN",
        deterministic: true,
        replaySafe: versionRuntime.replaySafe,
        sourceVersion: ADVANCED_INTELLIGENCE_V2_RUNTIME_VERSION,
    ...
    const observability = createObservabilityEvent({
      eventType: "ADVANCED_INTELLIGENCE_V2_ERROR",
      domain: "runtime",
      severity: "ERROR",
      message:
        "Advanced Intelligence v2 API encountered an unhandled runtime error.",
      traceId,
      replayRef: traceId,
      module: "api.governance.advanced-intelligence-v2",
      metadata: {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Unknown Advanced Intelligence v2 runtime error.",
      },
    });
