...
function programNote(name: string) {
  ...
  if (value.includes("construction") || value.includes("renovation")) return { fit: "Commercial acquisition, renovation, or value-add financing", why: "Commercial lenders commonly finance acquisition and approved improvements together through a bank portfolio loan, SBA structure, bridge facility, or construction-to-permanent execution; it is not automatically a separate residential-style construction loan.", watch: "Furlong sizes the property/project case from purchase price, stabilized value, renovation budget, debt-service coverage, environmental condition, permits, contractor controls, and occupancy timing. The selected lender separately handles any borrower-side underwriting it requires; personal financials do not affect Furlong's nonresidential property ranking." };
