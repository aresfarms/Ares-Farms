...
export async function GET(req: NextRequest) { const tester = actor(req); if (!tester) return NextResponse.json({ ok: false, error: "Named tester identity required." }, { status: 403 }); try { return NextResponse.json({ ok: true, currentTester: tester, rollup: await buildRollup() }); } catch (error) { return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Acceptance is unavailable." }, { status: 503 }); } }
export async function POST(req: NextRequest) {
  ...
  try { const attestation = await recordAttestation({ testerEmail: tester, verdict: body.verdict!, findings }); return NextResponse.json({ ok: true, attestation, rollup: await buildRollup() }); }
