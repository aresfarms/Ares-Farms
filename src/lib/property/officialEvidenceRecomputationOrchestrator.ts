...
export type RecomputationHandler=(artifact:DownstreamArtifactRecord)=>Promise<void>|void;
export async function processEvidenceRecomputationQueue(handlers:Partial<Record<DownstreamArtifactKind,RecomputationHandler>>,now=()=>new Date().toISOString()):Promise<RecomputationJob[]>{
 ...
 for(const job of jobs.filter(j=>j.status==="queued"||j.status==="blocked").sort((a,b)=>a.order-b.order)){
  ...
  try{await handler(artifact); const dependencies=currentOfficialEvidenceDependencies(); if(dependencies.length===0) throw new Error("No current published official evidence dependencies are available."); recomputeDownstreamArtifact({artifactId:job.artifactId,dependencies,at:now()}); readDownstreamArtifactForServe(job.artifactId); jobs[index]={...jobs[index],status:"completed",finishedAt:now(),reason:"Recomputed and verified against current official evidence."};}
