...
async function main() {
  ...
  const home = await fetch(BASE, { signal: AbortSignal.timeout(2500) })
    ...
    .catch(() => null);
  const reachable = !!home && home.status === 200 && /Furlong/.test(home.body);
  if (!reachable) {
    console.log("  (server not reachable — HTTP refresh checks skipped; module crash-safety checks ran)");
  } else if (!/nonce="[A-Za-z0-9+/=]{16,}"/.test(home.body)) {
    // Production-build test: a dev server serves a relaxed CSP with no per-script
    // nonce, so the nonce assertions can't hold. Skip cleanly on dev — run
    // against `next start` via BASE_URL for the real check (never a false red).
    console.log("  (dev-mode server: no nonce-CSP — HTTP refresh checks skipped; run against `next start` via BASE_URL)");
  } else {
    ...
    for (const r of routes) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        // DISCOVERY-PRIMARY (founder-approved 2026-07): /discover and /navigator
        // both serve the place-first DiscoverSurface now; accept its marker or the
        // legacy navigator markers (mirrors verify:csp-hydration's front-door set).
        ok(/data-testid="place-first-discovery"|data-testid="navigator-input"|furlong-navigator/.test(body), `${r} load #${i + 1}: front-door surface server-rendered`);
      }
    }
  }
