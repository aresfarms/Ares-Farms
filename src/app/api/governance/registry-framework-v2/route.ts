...
type RegistryFrameworkV2Request = RegistryFrameworkV2Input;

function createRegistryFrameworkV2TraceId(): string {
  return `registry-framework-v2-${Date.now()}-${Math.random()
    .toString(36)
    .slice(2, 10)}`;
}

export async function POST(req: NextRequest) {
  const traceId = createRegistryFrameworkV2TraceId();

  try {
    const body = (await req
      .json()
      .catch(() => ({}))) as RegistryFrameworkV2Request;
    const actorId = body.userId ?? body.reviewerRole ?? null;

    const runtimeGuard = runRuntimeGuard({
      operation: "governance.registry.framework.v2.compose",
      module: "api.governance.registry-framework-v2",
      traceId,
      schemaVersion: "registry-framework-v2-request-v0.1.0",
      governanceVersion: "master-volumes-runtime-v0.1.0",
      classificationLevel: "RESTRICTED",
      replayRef: traceId,
      actorId,
      metadata: {
        route: "/api/governance/registry-framework-v2",
    ...
    if (!runtimeGuard.allowed) {
      const observability = createObservabilityEvent({
        eventType: "REGISTRY_FRAMEWORK_V2_RUNTIME_BLOCKED",
        domain: "runtime",
        severity: "WARN",
        message:
          "Registry Framework v2 runtime guard blocked the request.",
        traceId,
        replayRef: traceId,
        actorId,
        module: "api.governance.registry-framework-v2",
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "Runtime governance guard blocked Registry Framework v2 request.",
    ...
    const versionRuntime = evaluateVersionRuntime({
      ...
      versions: [
        createRuntimeVersionRef(
          "schema",
          "registry-framework-v2-request-v0.1.0",
          "src/app/api/governance/registry-framework-v2/route.ts",
          traceId
        ),
        createRuntimeVersionRef(
          "governance",
    ...
    const classifiedInput = classifyRecord(body as Record<string, unknown>, {
      ...
      classificationSource: "api-governance-registry-framework-v2-route",
      ...
      consentRequirements: [
        "governance-registry-framework-v2-review-consent",
      ],
    });

    const v2Result = composeRegistryFrameworkV2(body);

    const classifiedOutput = classifyRecord(
      {
        v2Result,
        event: {
          eventType: "governance.registry.framework.v2.composed",
      ...
      },
      {
        ...
        classificationSource:
          "api-governance-registry-framework-v2-route-output",
        classificationVersion: "classification-runtime-v0.1.0",
        replayRef: traceId,
        disclosureAudience: [
          "authorized-operator",
    ...
    const explanation = createExplanationLineage({
      ...
      outputType: "registry_framework_v2_pack",
      audience: "governance",
      claimType: "recommendation",
      summary:
        "Registry Framework v2 pack composed as advisory internal registry evidence, replay-safe, audit-safe, conflict-preserving internal evidence only. No external promotion, autonomous customer eligibility / pathway / opportunity / intelligence / evidence / certification / registry determination, credit decision, lender commitment, public verification, regulatory reliance, tax-credit allocation, environmental clearance, carbon-credit issuance, source certainty claim, or legal reliance is created.",
      ruleVersion: REGISTRY_FRAMEWORK_V2_RUNTIME_VERSION,
      overlayRefs: [],
      confidenceScore: Math.min(
        0.85,
        Math.max(0.45, 0.45 + v2Result.summary.v2EntryCount / 120)
      ),
      humanReviewRequired: true,
      replayRefs: [traceId],
      auditEventRefs: [],
      metadata: {
        v2CatalogCount: v2Result.summary.v2CatalogCount,
        v2EntryCount: v2Result.summary.v2EntryCount,
        legacyCatalogCount: v2Result.summary.legacyCatalogCount,
        legacyTotalEntryCount: v2Result.summary.legacyTotalEntryCount,
        legacyProductionBlockedEntryCount:
          v2Result.summary.legacyProductionBlockedEntryCount,
        crossSourceConflictCount:
          v2Result.summary.crossSourceConflictCount,
        certificationV2OverallReadinessPercent:
          v2Result.summary.certificationV2OverallReadinessPercent,
        certificationV1OverallReadinessPercent:
          v2Result.summary.certificationV1OverallReadinessPercent,
    ...
    const observability = createObservabilityEvent({
      eventType: "REGISTRY_FRAMEWORK_V2_COMPOSED",
      domain: "operations",
      severity: "INFO",
      message:
        "Registry Framework v2 pack composed through governed runtime controls.",
      traceId,
      replayRef: traceId,
      actorId,
      module: "api.governance.registry-framework-v2",
      metadata: {
        ...
        v2CatalogCount: v2Result.summary.v2CatalogCount,
        v2EntryCount: v2Result.summary.v2EntryCount,
        v2CapitalProgramEntryCount:
          v2Result.summary.v2CapitalProgramEntryCount,
        v2CustomerTypeEntryCount:
          v2Result.summary.v2CustomerTypeEntryCount,
        v2CapitalCategoryEntryCount:
          v2Result.summary.v2CapitalCategoryEntryCount,
        v2CertificationPostureEntryCount:
          v2Result.summary.v2CertificationPostureEntryCount,
        legacyCatalogCount: v2Result.summary.legacyCatalogCount,
        legacyTotalEntryCount: v2Result.summary.legacyTotalEntryCount,
        crossSourceConflictCount:
          v2Result.summary.crossSourceConflictCount,
    ...
    const evidence = await persistGovernanceEvidence({
      ...
      classifications: [
        {
          resourceType: "registry_framework_v2_input",
          resourceId: traceId,
        ...
        {
          resourceType: "registry_framework_v2_output",
          resourceId: traceId,
      ...
      replayVerification: {
        traceId,
        replayRef: traceId,
        targetType: "registry_framework_v2_pack",
        targetId: body.applicationId ?? traceId,
        verificationStatus: versionRuntime.ok ? "PASS" : "WARN",
        deterministic: true,
        replaySafe: versionRuntime.replaySafe,
        sourceVersion: REGISTRY_FRAMEWORK_V2_RUNTIME_VERSION,
    ...
    const observability = createObservabilityEvent({
      eventType: "REGISTRY_FRAMEWORK_V2_ERROR",
      domain: "runtime",
      severity: "ERROR",
      message:
        "Registry Framework v2 API encountered an unhandled runtime error.",
      traceId,
      replayRef: traceId,
      module: "api.governance.registry-framework-v2",
      metadata: {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Unknown Registry Framework v2 runtime error.",
      },
    });
