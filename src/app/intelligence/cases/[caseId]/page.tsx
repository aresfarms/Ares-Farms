...
export default async function IntelligenceCasePage({
  params,
  searchParams,
}: {
  ...
  const workspace = composeIntelligenceCaseWorkspace({
    ...
    customerTypes,
    intendedUses,
    reviewReceipt: read("reviewStatus")
      ? {
          reviewId: read("reviewId"),
          status: read("reviewStatus") === "COMPLETED" ? "COMPLETED" : "QUEUED",
          outcome: read("reviewOutcome"),
          finalActionAllowed:
            read("finalActionAllowed") === "true"
              ? true
              : read("finalActionAllowed") === "false"
                ? false
                : null,
        }
      : null,
  ...
  const humanReviewParams = new URLSearchParams(onboardingParams);
  humanReviewParams.set("origin", "intelligence-case");
      ...
      >
        <header style={panel}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <span>·</span>
            <span>{workspace.evidence.length} evidence signals</span>
            <span>·</span>
            <span>{workspace.scenarios.length} scenarios</span>
            <span>·</span>
            <span>{workspace.conflictCount} conflicts</span>
          </div>
        </header>
        ...
        <section style={{ ...panel, borderLeft: "5px solid #c58b21" }}>
          ...
          <p style={muted}>
            ...
            determine eligibility, certify property facts, or execute an
            external action.
          </p>
        </section>
        ...
        <section>
          ...
          >
            {workspace.scenarios.map((scenario) => (
              <article key={scenario.scenarioId} style={panel}>
                <h3 style={{ marginTop: 0 }}>{scenario.label}</h3>
                <p style={muted}>{scenario.operatingPosture}</p>
                <div>
                  <strong>Compliance:</strong> {scenario.compliancePosture}
                </div>
                <div>
                  <strong>Downside:</strong> {scenario.downsidePosture}
                </div>
                <div>
                  <strong>Unresolved decisions:</strong>{" "}
                  {scenario.unresolvedDecisionIds.length}
                </div>
              </article>
            ))}
          </div>
        ...
        <section style={panel}>
          ...
          {workspace.evidence.length === 0 ? (
            <p style={muted}>
              ...
              type, intended use, jurisdiction, property, and documents to
              deepen the case.
            </p>
          ) : (
            <ul>
              {workspace.evidence.slice(0, 24).map((item) => (
        ...
        <section style={panel}>
          ...
          <p style={muted}>
            ...
            {workspace.outcome.status === "NOT_STARTED"
              ? "Outcome capture has not begun. This case will later compare the released recommendation, the human choice, and the actual result."
              : "Human-review activity has been attached to this case. Actual-world outcome capture remains separate and has not been completed."}
          </p>
          {workspace.outcome.varianceNotes.length > 0 ? (
            <ul>
              {workspace.outcome.varianceNotes.map((note) => (
                <li key={note}>{note}</li>
              ))}
            </ul>
          ) : null}
        ...
        <footer style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
          <Link href={`/onboarding?${onboardingParams.toString()}`}>
            Enrich this case through onboarding
          </Link>
          <Link href={`/navigator?${navigatorParams.toString()}`}>
            Enrich this case through Navigator
          </Link>
          <Link
            href={`/governance/advanced-intelligence-v2?${governedReviewParams.toString()}`}
          >
            Open governed intelligence review
          </Link>
          <Link href={`/reviews?${humanReviewParams.toString()}`}>
            Open human review
          </Link>
        </footer>
      </div>
