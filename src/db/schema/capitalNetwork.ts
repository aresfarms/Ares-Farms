...
export const capitalNetworkProviders = pgTable(
  "capital_network_providers",
  {
    ...
    publishedCreditBox: jsonb("published_credit_box").notNull().default({}),
    collateralPolicy: jsonb("collateral_policy").notNull().default({}),
    environmentalRequirements: jsonb("environmental_requirements").notNull().default({}),
    typicalFirstResponseDays: integer("typical_first_response_days"),
    typicalClosingDays: integer("typical_closing_days"),
    creditBoxSourceRefs: jsonb("credit_box_source_refs").notNull().default([]),
    creditBoxVerifiedAt: timestamp("credit_box_verified_at", { withTimezone: true }),
...
export const capitalNetworkDealRooms = pgTable(
  "capital_network_deal_rooms",
  {
    ...
    dataShared: boolean("data_shared").notNull().default(false),
    caseRoomExpiresAt: timestamp("case_room_expires_at", { withTimezone: true }),
    providerResponseStatus: text("provider_response_status"),
    providerResponseSummary: text("provider_response_summary"),
    providerResponseDetails: jsonb("provider_response_details"),
    providerRespondedAt: timestamp("provider_responded_at", { withTimezone: true }),
    closingMilestones: jsonb("closing_milestones").notNull().default([]),
    selectedAt: timestamp("selected_at", { withTimezone: true }),
