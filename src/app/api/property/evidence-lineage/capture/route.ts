...
export async function POST(request: NextRequest) {
  const body = await request.json().catch(() => null) as { kind?: string; propertyId?: string; artifactId?: string; replayInput?: unknown; replayOutput?: unknown } | null;
  ...
  if (body?.replayInput === undefined || body?.replayOutput === undefined) return NextResponse.json({ ok:false, error:"Signed replay input and output are required." }, { status:400 });
  const artifact = captureGeneratedEvidenceArtifact({ kind, propertyId, artifactId, replayInput:body.replayInput, replayOutput:body.replayOutput });
