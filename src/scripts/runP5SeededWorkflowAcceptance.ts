...
const TARGET_REVISION = process.env.P5_TARGET_REVISION ?? "furlong-core-00094-b5b";
const EXPECTED_IMAGE = process.env.P5_TARGET_IMAGE_DIGEST ?? "sha256:10e00056b431a7a0d437698ca58f89d836f7242f88a2a89f5c45b63e93755ca6";
...
async function main(): Promise<void> {
  ...
  const revisionImage = gcloud(["run", "revisions", "describe", TARGET_REVISION, "--region", REGION, "--format", "value(spec.containers[0].image)"]);
  const revisionReady = gcloud(["run", "revisions", "describe", TARGET_REVISION, "--region", REGION, "--format", "value(status.conditions[0].status)"]);
  ...
  add("target revision ready", revisionReady === "True", revisionReady, TARGET_REVISION);
  add("target image digest exact", revisionImage.endsWith(`@${EXPECTED_IMAGE}`), revisionImage, EXPECTED_IMAGE);
  add("P4 seed bound to target revision", seedValue.cloudRunRevision === TARGET_REVISION, String(seedValue.cloudRunRevision), seed.file);
  ...
  const logFilter = `resource.type=\"cloud_run_revision\" AND resource.labels.service_name=\"${SERVICE}\" AND resource.labels.revision_name=\"${TARGET_REVISION}\" AND jsonPayload.policy=\"staging-seed-authority\"`;
  ...
  const acceptance = {
    ...
    targetImageDigest: EXPECTED_IMAGE,
  ...
  const blockerReport = { schemaVersion: "p5-production-blocker-ledger-v1", environment: "staging", targetRevision: TARGET_REVISION, productionAuthorized: false, blockers, generatedAtUtc };
  ...
  const acceptancePath = path.join(dir, `${stamp}-${TARGET_REVISION}-p5-acceptance.json`);
  const blockersPath = path.join(dir, `${stamp}-${TARGET_REVISION}-p5-blockers.json`);
  ...
  const signaturePath = path.join(dir, `${stamp}-${TARGET_REVISION}-p5-signature.json`);
