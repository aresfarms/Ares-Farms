  ...
  return <main style={{maxWidth:1100,margin:"0 auto",padding:"32px 20px",fontFamily:"system-ui"}}>
    ...
    {snapshot ? <>
      ...
      {snapshot.report ? <>
        ...
        <section><h2>Current blockers</h2>{snapshot.report.blockers.length ? <ul>{snapshot.report.blockers.map((x:string)=><li key={x}>{x}</li>)}</ul> : <p>None in the frozen report.</p>}</section><section><h2>Pilot testing boundary</h2><p>The owner may green-light controlled internal testing after the owner attestation. This never permits public launch, external actions, payment capture, notice delivery, or official reliance. Independent review remains required for final launch.</p>{snapshot.pilotTestDecision?.blockers?.length ? <ul>{snapshot.pilotTestDecision.blockers.map((x:string)=><li key={x}>{x}</li>)}</ul> : <p>Pilot gate has no blockers.</p>}</section>
      </> : null}
