import { NextRequest, NextResponse } from "next/server";

import { persistGovernanceEvidence } from "@/lib/governance/evidenceStore";
import {
  REVENUE_INTELLIGENCE_V2_RUNTIME_VERSION,
  RevenueIntelligenceV2Input,
  composeRevenueIntelligenceV2,
} from "@/lib/revenue-intelligence/revenueIntelligenceV2Runtime";
import { classifyRecord } from "@/lib/runtime/classificationRuntime";
import { createExplanationLineage } from "@/lib/runtime/explainabilityRuntime";
import { createObservabilityEvent } from "@/lib/runtime/observabilityRuntime";
import { runRuntimeGuard } from "@/lib/runtime/runtimeGuard";
import {
  createRuntimeVersionRef,
  evaluateVersionRuntime,
} from "@/lib/runtime/versionRuntime";

/**
 * Revenue Intelligence v2 API
 *
 * Master Volume Governance:
 * - Vol I: preserves Customer Type review boundaries and Capital Graph
 *   sponsor authority; the route never grants authority.
 * - Vol II: every composed program-level review routes to the named
 *   sponsor / regulatory authority.
 * - Vol III: deterministic, replay-safe composition with explicit
 *   version lineage chaining v2 → Customer Type → Capital Graph → v1
 *   revenue-source-intelligence.
 * - Vol III-B: runtime guard, classification, version lineage,
 *   observability, explainability, replay verification, audit-safe
 *   error envelope.
 * - Vol IV: routes governed handoffs to the Capital Graph, Customer
 *   Type Registry, financing pathway guidance, opportunity discovery,
 *   advanced intelligence, lender workflow, evidence engine,
 *   certification engine, registry framework, governance, and reviews.
 * - Vol V: preserves canonical claims governance, controlled disclosure,
 *   replay, audit, portability, and source-authority boundaries.
 * - Vol VI: keeps every composed entry behind a public-safe DTO; no raw
 *   sponsor or borrower records, no live external fetch, no source-
 *   certainty claim.
 */

type RevenueIntelligenceV2Request = RevenueIntelligenceV2Input;

function createRevenueIntelligenceV2TraceId(): string {
  return `revenue-intelligence-v2-${Date.now()}-${Math.random()
    .toString(36)
    .slice(2, 10)}`;
}

export async function POST(req: NextRequest) {
  const traceId = createRevenueIntelligenceV2TraceId();

  try {
    const body = (await req
      .json()
      .catch(() => ({}))) as RevenueIntelligenceV2Request;
    const actorId = body.userId ?? body.reviewerRole ?? null;

    const runtimeGuard = runRuntimeGuard({
      operation: "governance.revenue.intelligence.v2.compose",
      module: "api.governance.revenue-intelligence-v2",
      traceId,
      schemaVersion: "revenue-intelligence-v2-request-v0.1.0",
      governanceVersion: "master-volumes-runtime-v0.1.0",
      classificationLevel: "RESTRICTED",
      replayRef: traceId,
      actorId,
      metadata: {
        route: "/api/governance/revenue-intelligence-v2",
        applicationId: body.applicationId ?? null,
      },
    });

    if (!runtimeGuard.allowed) {
      const observability = createObservabilityEvent({
        eventType: "REVENUE_INTELLIGENCE_V2_RUNTIME_BLOCKED",
        domain: "runtime",
        severity: "WARN",
        message:
          "Revenue Intelligence v2 runtime guard blocked the request.",
        traceId,
        replayRef: traceId,
        actorId,
        module: "api.governance.revenue-intelligence-v2",
        metadata: {
          route: "/api/governance/revenue-intelligence-v2",
          findings: runtimeGuard.findings,
        },
      });

      const evidence = await persistGovernanceEvidence({
        traceId,
        replayRef: traceId,
        observability,
        metadata: {
          route: "/api/governance/revenue-intelligence-v2",
          runtimeBlocked: true,
        },
      });

      return NextResponse.json(
        {
          ok: false,
          error:
            "Runtime governance guard blocked Revenue Intelligence v2 request.",
          governance: {
            traceId,
            runtimeGuard,
            observability,
            evidence,
          },
        },
        { status: 403 }
      );
    }

    const versionRuntime = evaluateVersionRuntime({
      operation: "governance.revenue.intelligence.v2.compose",
      module: "api.governance.revenue-intelligence-v2",
      traceId,
      versions: [
        createRuntimeVersionRef(
          "schema",
          "revenue-intelligence-v2-request-v0.1.0",
          "src/app/api/governance/revenue-intelligence-v2/route.ts",
          traceId
        ),
        createRuntimeVersionRef(
          "governance",
          "master-volumes-runtime-v0.1.0",
          "Master Volume Series",
          traceId
        ),
        createRuntimeVersionRef(
          "runtime",
          "runtime-enforcement-v0.1.0",
          "src/lib/runtime",
          traceId
        ),
        createRuntimeVersionRef(
          "rules",
          REVENUE_INTELLIGENCE_V2_RUNTIME_VERSION,
          "src/lib/revenue-intelligence/revenueIntelligenceV2Runtime.ts",
          traceId
        ),
        createRuntimeVersionRef(
          "rules",
          "customer-type-runtime-v0.1.0",
          "src/lib/customer-types/customerTypeRuntime.ts",
          traceId
        ),
        createRuntimeVersionRef(
          "rules",
          "capital-graph-runtime-v0.1.0",
          "src/lib/capital-graph/capitalGraphRuntime.ts",
          traceId
        ),
        createRuntimeVersionRef(
          "runtime",
          "revenue-source-intelligence-runtime-v0.1.0",
          "src/lib/revenue-intelligence/revenueSourceIntelligenceRuntime.ts",
          traceId
        ),
      ],
    });

    const classifiedInput = classifyRecord(body as Record<string, unknown>, {
      classificationLevel: "RESTRICTED",
      sensitivityScope: "governance",
      classificationSource: "api-governance-revenue-intelligence-v2-route",
      classificationVersion: "classification-runtime-v0.1.0",
      replayRef: traceId,
      disclosureAudience: [
        "authorized-operator",
        "governance",
        "auditor",
        "regulator",
      ],
      sharingPermissions: [
        "regulated-operational-review",
        "governance-evidence-review",
      ],
      aiUsagePermissions: ["summarize", "classify", "explain"],
      exportRestrictions: [
        "requires-governed-access",
        "not-an-approval",
        "not-an-eligibility-determination",
        "not-a-credit-decision",
        "not-a-public-verification",
        "not-a-regulatory-reliance",
        "not-a-lender-commitment",
        "not-a-live-external-action",
        "requires-human-review",
      ],
      redactionRequirements: [
        "redact-sensitive-borrower-content-before-external-disclosure",
        "redact-sovereign-participant-records-before-external-disclosure",
      ],
      consentRequirements: [
        "governance-revenue-intelligence-v2-review-consent",
      ],
    });

    const v2Result = composeRevenueIntelligenceV2(body);

    const classifiedOutput = classifyRecord(
      {
        v2Result,
        event: {
          eventType: "governance.revenue.intelligence.v2.composed",
          applicationId: v2Result.applicationId,
          replayRef: traceId,
          humanReviewRequired: true,
        },
      },
      {
        classificationLevel: "RESTRICTED",
        sensitivityScope: "governance",
        classificationSource:
          "api-governance-revenue-intelligence-v2-route-output",
        classificationVersion: "classification-runtime-v0.1.0",
        replayRef: traceId,
        disclosureAudience: [
          "authorized-operator",
          "governance",
          "auditor",
          "regulator",
        ],
        sharingPermissions: [
          "regulated-operational-review",
          "governance-evidence-review",
        ],
        aiUsagePermissions: ["summarize", "explain"],
        exportRestrictions: [
          "not-an-approval",
          "not-an-eligibility-determination",
          "not-a-credit-decision",
          "not-a-public-verification",
          "not-a-regulatory-reliance",
          "not-a-lender-commitment",
          "not-a-program-approval",
          "not-a-tax-credit-allocation",
          "not-an-environmental-clearance",
          "not-a-carbon-credit-issuance",
          "not-a-live-external-action",
          "not-a-legal-reliance",
          "requires-human-review",
        ],
        redactionRequirements: [
          "redact-internal-review-notes-before-public-disclosure",
          "redact-sovereign-participant-records-before-external-disclosure",
        ],
        consentRequirements: [
          "governance-revenue-intelligence-v2-review-consent",
        ],
      }
    );

    const explanation = createExplanationLineage({
      outputIdentifier: traceId,
      outputType: "revenue_intelligence_v2_pack",
      audience: "governance",
      claimType: "recommendation",
      summary:
        "Revenue Intelligence v2 pack composed as advisory, replay-safe, audit-safe, conflict-preserving internal evidence only. No approval, autonomous customer eligibility determination, credit decision, lender commitment, public verification, regulatory reliance, tax-credit allocation, environmental clearance, carbon-credit issuance, or legal reliance is created.",
      ruleVersion: REVENUE_INTELLIGENCE_V2_RUNTIME_VERSION,
      overlayRefs: [],
      confidenceScore: Math.min(
        0.85,
        Math.max(
          0.45,
          0.45 + v2Result.summary.totalComposedProgramCount / 60
        )
      ),
      humanReviewRequired: true,
      replayRefs: [traceId],
      auditEventRefs: [],
      metadata: {
        customerProfileCount: v2Result.summary.customerProfileCount,
        totalComposedProgramCount:
          v2Result.summary.totalComposedProgramCount,
        totalLegacyOpportunityCount:
          v2Result.summary.totalLegacyOpportunityCount,
        conflictSignalCount: v2Result.summary.conflictSignalCount,
        crossSourceConflictCount:
          v2Result.summary.crossSourceConflictCount,
        sovereignProgramCount: v2Result.summary.sovereignProgramCount,
        replaySafe: v2Result.replaySafe,
        auditSafe: v2Result.auditSafe,
        conflictPreserving: v2Result.conflictPreserving,
        federationScoped: v2Result.federationScoped,
        productionBlocked: v2Result.productionBlocked,
      },
    });

    const observability = createObservabilityEvent({
      eventType: "REVENUE_INTELLIGENCE_V2_COMPOSED",
      domain: "operations",
      severity: "INFO",
      message:
        "Revenue Intelligence v2 pack composed through governed runtime controls.",
      traceId,
      replayRef: traceId,
      actorId,
      module: "api.governance.revenue-intelligence-v2",
      metadata: {
        route: "/api/governance/revenue-intelligence-v2",
        customerProfileCount: v2Result.summary.customerProfileCount,
        totalComposedProgramCount:
          v2Result.summary.totalComposedProgramCount,
        totalLegacyOpportunityCount:
          v2Result.summary.totalLegacyOpportunityCount,
        crossSourceConflictCount:
          v2Result.summary.crossSourceConflictCount,
        sovereignProgramCount: v2Result.summary.sovereignProgramCount,
        participantProgramCount:
          v2Result.summary.participantProgramCount,
        publicProgramCount: v2Result.summary.publicProgramCount,
        versionRuntimeOk: versionRuntime.ok,
        classificationLevel:
          classifiedOutput.classification.classificationLevel,
      },
    });

    const evidence = await persistGovernanceEvidence({
      traceId,
      replayRef: traceId,
      versionRuntime,
      classifications: [
        {
          resourceType: "revenue_intelligence_v2_input",
          resourceId: traceId,
          classification: classifiedInput.classification,
          traceId,
          replayRef: traceId,
          metadata: {
            route: "/api/governance/revenue-intelligence-v2",
            stage: "input",
            applicationId: body.applicationId ?? null,
          },
        },
        {
          resourceType: "revenue_intelligence_v2_output",
          resourceId: traceId,
          classification: classifiedOutput.classification,
          traceId,
          replayRef: traceId,
          metadata: {
            route: "/api/governance/revenue-intelligence-v2",
            stage: "output",
            advisoryOnly: true,
            replaySafe: true,
            auditSafe: true,
            conflictPreserving: true,
            federationScoped: true,
            productionBlocked: true,
          },
        },
      ],
      observability,
      replayVerification: {
        traceId,
        replayRef: traceId,
        targetType: "revenue_intelligence_v2_pack",
        targetId: body.applicationId ?? traceId,
        verificationStatus: versionRuntime.ok ? "PASS" : "WARN",
        deterministic: true,
        replaySafe: versionRuntime.replaySafe,
        sourceVersion: REVENUE_INTELLIGENCE_V2_RUNTIME_VERSION,
        replayVersion: "governance-evidence-store-v0.1.0",
        eventCount: 1,
        mismatchCount: versionRuntime.ok ? 0 : 1,
        result: {
          customerProfileCount: v2Result.summary.customerProfileCount,
          totalComposedProgramCount:
            v2Result.summary.totalComposedProgramCount,
          totalLegacyOpportunityCount:
            v2Result.summary.totalLegacyOpportunityCount,
          versionRuntimeOk: versionRuntime.ok,
        },
        metadata: {
          route: "/api/governance/revenue-intelligence-v2",
          operation: "governance.revenue.intelligence.v2.compose",
        },
      },
      metadata: {
        route: "/api/governance/revenue-intelligence-v2",
        operation: "governance.revenue.intelligence.v2.compose",
      },
    });

    return NextResponse.json({
      ok: true,
      v2Result,
      event: classifiedOutput.event,
      governance: {
        traceId,
        runtimeGuard,
        versionRuntime,
        inputClassification: classifiedInput.classification,
        outputClassification: classifiedOutput.classification,
        explainability: explanation,
        observability,
        evidence,
      },
    });
  } catch (error) {
    const observability = createObservabilityEvent({
      eventType: "REVENUE_INTELLIGENCE_V2_ERROR",
      domain: "runtime",
      severity: "ERROR",
      message:
        "Revenue Intelligence v2 API encountered an unhandled runtime error.",
      traceId,
      replayRef: traceId,
      module: "api.governance.revenue-intelligence-v2",
      metadata: {
        route: "/api/governance/revenue-intelligence-v2",
        error:
          error instanceof Error
            ? error.message
            : "Unknown Revenue Intelligence v2 runtime error.",
      },
    });

    const evidence = await persistGovernanceEvidence({
      traceId,
      replayRef: traceId,
      observability,
      metadata: {
        route: "/api/governance/revenue-intelligence-v2",
        runtimeError: true,
      },
    });

    return NextResponse.json(
      {
        ok: false,
        error:
          error instanceof Error
            ? error.message
            : "Unknown Revenue Intelligence v2 runtime error.",
        governance: {
          traceId,
          observability,
          evidence,
        },
      },
      { status: 500 }
    );
  }
}
