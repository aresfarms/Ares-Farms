...
export const INSTITUTIONAL_APPEAL_FINALITY_SCHEMA_VERSION = "institutional-appeal-finality-v1";
export type AppealFinalityDecision = "ALLOW" | "BLOCK" | "REVIEW_REQUIRED";
export type AppealFinalityState = "FINALIZED" | "REVIEW_PENDING" | "BLOCKED";

export type InstitutionalAppealFinalityPolicy = Readonly<{
...
export type InstitutionalAppealFinalityEvaluation = Readonly<{
  schemaVersion:typeof INSTITUTIONAL_APPEAL_FINALITY_SCHEMA_VERSION;
  dischargePolicyId:string; finalityPolicyId:string; reconciliationId:string;
  decision:AppealFinalityDecision; resultingState:AppealFinalityState;
  reasons:readonly string[]; immutableRecordRefs:readonly string[];
  evidenceRefs:readonly string[]; finalizedAt:string;
...
export function createInstitutionalAppealFinalityPolicy(input:InstitutionalAppealFinalityPolicy):InstitutionalAppealFinalityPolicy{
...
export function evaluateInstitutionalAppealFinality(input:{discharge:InstitutionalAppealMonitoringDischargeEvaluation;policy:InstitutionalAppealFinalityPolicy;immutableRecordRefs?:readonly string[];retentionPolicyRef?:string|null;legalHoldAssessmentRef?:string|null;closureNoticeRef?:string|null;reopeningAuthorityRef?:string|null;evidenceRefs:readonly string[];finalizedAt:string;auditRefs:readonly string[];replayRef:string;}):InstitutionalAppealFinalityEvaluation{
  iso(input.finalizedAt,"finalizedAt");const p=createInstitutionalAppealFinalityPolicy(input.policy);const records=list(input.immutableRecordRefs??[],"immutableRecordRefs");const evidence=list(input.evidenceRefs,"evidenceRefs");const reasons:string[]=[];
  if(input.discharge.decision!=="ALLOW"||input.discharge.resultingState!=="DISCHARGED")reasons.push("allowed-discharge-required");
  ...
  const decision:AppealFinalityDecision=hard?"BLOCK":reasons.length?"REVIEW_REQUIRED":"ALLOW";
  return Object.freeze({schemaVersion:INSTITUTIONAL_APPEAL_FINALITY_SCHEMA_VERSION,dischargePolicyId:input.discharge.dischargePolicyId,finalityPolicyId:p.policyId,reconciliationId:input.discharge.reconciliationId,decision,resultingState:decision==="ALLOW"?"FINALIZED":decision==="REVIEW_REQUIRED"?"REVIEW_PENDING":"BLOCKED",reasons:Object.freeze([...new Set(reasons)].sort()),immutableRecordRefs:records,evidenceRefs:evidence,finalizedAt:input.finalizedAt,auditRefs:list(input.auditRefs,"auditRefs",true),replayRef:ne(input.replayRef,"replayRef")});
}

export const institutionalAppealFinalityAuthority=Object.freeze({createPolicy:createInstitutionalAppealFinalityPolicy,evaluate:evaluateInstitutionalAppealFinality});
