...
const CANDIDATES: Candidate[] = [
 ...
 {key:"hay-pasture",label:"Bulk hay and managed pasture",acresShare:.7,grossPerAcre:520,costPct:.55,startupPerAcre:220,labor:30,water:30,market:35,yearsToCash:1,note:"Generic bulk-forage or pasture benchmark; not a premium small-square-bale model."},
 {key:"alfalfa-small-square",label:"Irrigated alfalfa — premium small squares",acresShare:.7,grossPerAcre:0,costPct:0,startupPerAcre:650,labor:82,water:78,market:72,yearsToCash:1,note:"Bale-level model using irrigated yield, bale weight, seasonal prices, handling, storage, and shrink."},
...
export function optimizeAgriculturalOpportunities(a:OpportunityAssumptions){
 const ranked=CANDIDATES.map(c=>{
  ...
  const opex=c.key==="alfalfa-small-square"
  ...
  return {...c,eligible,usedAcres,gross,opex,noi,startup,fit,returnOnStartup,dscr:a.debtService>0?noi/a.debtService:null,modelDetails:c.key==="alfalfa-small-square"?{hayYield,baleWeight,balesPerTon,sellableBales,averageBalePrice,winterShare,shrink}:null};
