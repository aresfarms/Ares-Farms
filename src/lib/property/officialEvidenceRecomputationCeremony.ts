...
export interface RecomputationActivationReceipt {
  ...
  at: string;
  reason: string;
  readyAtDecision: boolean;
  handlerHashes: Record<string, string | null>;
}
interface State {
  receipts: RecomputationActivationReceipt[];
}
const FILE = runtimeStatePath(
  "official-evidence",
  "recomputation-activation-ceremony.json",
);
const read = (): State => {
  try {
...
export function recordRecomputationActivationCeremony(input: {
  action: CeremonyAction;
  actorId: string;
  actorName: string;
  reason: string;
  at?: string;
}): RecomputationActivationReceipt {
  if (!input.actorId.trim() || !input.actorName.trim() || !input.reason.trim())
    throw new Error(
      "Activation ceremony requires an attributed actor and reason.",
    );
  ...
  if (
    ...
    throw new Error(
      "Activation cannot be finalized until the current four-decision packet is complete, all four items are approved, and all exact handlers are replay-matched and runtime-bound.",
    );
  const receipt: RecomputationActivationReceipt = {
    ...
    reason: input.reason,
    readyAtDecision: activation.ready,
    handlerHashes: Object.fromEntries(
      activation.details.map((item) => [item.kind, item.implementationHash]),
    ),
  };
  const state = read();
...
export function listRecomputationActivationReceipts(): RecomputationActivationReceipt[] {
  return read().receipts;
}
export function recomputationActivationFinalized(): boolean {
  const receipts = read().receipts;
  const latestFinal = [...receipts]
    .reverse()
    .find(
      (receipt) => receipt.action === "FINALIZE" || receipt.action === "REVOKE",
    );
