...
assert(r.required===18,"All 18 authority slots are required.");
...
assert(r.slots.filter((slot)=>slot.assignment?.status==="ASSIGNED").length===2,"Exactly two launch slots should currently be assigned.");
...
assert(!actorMayUseStagingUltimateAuthority("unlisted@example.com", stagingOverrideEnv),"Unlisted identities must not receive staging ultimate authority.");
