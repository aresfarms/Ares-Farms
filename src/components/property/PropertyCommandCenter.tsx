...
function financingProgramNote(name: string, lane: string) {
  ...
  if (value.includes("va")) return { fit: "Veteran owner-occupant path", why: "Potentially strong when an eligible veteran intends to occupy the property.", watch: "COE, borrower qualification, VA appraisal/minimum-property requirements, and renovation-lender availability are handled in the personalized Financial module." };
  if (value.includes("conventional")) return { fit: "Standard residential financing", why: "Useful when appraisal and condition support ordinary mortgage collateral requirements.", watch: "A teardown or major asbestos-related rehabilitation may require renovation or construction financing instead." };
  ...
  if (value.includes("usda")) return { fit: "Rural-property pathway", why: `Potential fit because this is a ${lane.toLowerCase()} property in a rural-market context.`, watch: "Program geography, occupancy or business purpose, household/borrower eligibility, appraisal, and use-of-proceeds rules must be confirmed." };
