...
export interface OfficialEvidenceConnectorRegistration {
  ...
  geographicScope: string[]; parserVersion: string; sourceUrl: string; implementationHash?: string; registeredAt: string; status: ConnectorApprovalStatus;
...
const implementationHash = (fetcher: OfficialEvidenceConnectorFetcher): string => createHash("sha256").update(fetcher.toString()).digest("hex");
...
function validate(r: OfficialEvidenceConnectorRegistration): void {
  ...
  if (r.implementationHash && !/^[a-f0-9]{64}$/.test(r.implementationHash)) throw new Error("Connector implementation hash must be a lowercase SHA-256 digest.");
  if (r.status === "approved" && (!r.reviewedBy?.trim() || !r.reviewedAt?.trim() || !r.implementationHash)) throw new Error("Approved connector registration requires reviewer identity, review timestamp, and implementation hash.");
}
...
const materialFingerprint = (r: OfficialEvidenceConnectorRegistration): string => JSON.stringify({
  ...
  legalBasis: r.legalBasis, geographicScope: [...r.geographicScope].sort(), parserVersion: r.parserVersion, sourceUrl: r.sourceUrl, implementationHash: r.implementationHash ?? null,
});
...
export function registerOfficialEvidenceConnector(registration: OfficialEvidenceConnectorRegistration, fetcher: OfficialEvidenceConnectorFetcher): void {
  const boundRegistration: OfficialEvidenceConnectorRegistration = {...registration, implementationHash: implementationHash(fetcher)};
  validate(boundRegistration);
  ...
  const materiallyChanged=!!previous && materialFingerprint(previous)!==materialFingerprint(boundRegistration);
  const next: OfficialEvidenceConnectorRegistration = materiallyChanged
    ? {...boundRegistration,status:"pending",reviewedBy:null,reviewedAt:null,reviewReason:"Material connector or implementation change requires fresh review."}
    : boundRegistration;
  ...
  const receipt: ConnectorReviewReceipt={receiptId:randomUUID(),connectorId:next.connectorId,sourceId:next.sourceId,decision:materiallyChanged?"CHANGE_REVIEW_REQUIRED":"REGISTER",actorId:"system:connector-registration",actorName:"connector-registration",decidedAt:next.registeredAt,reason:materiallyChanged?"Material connector identity, authority, legal basis, scope, parser, URL, or implementation hash changed; approval reset to pending.":"Connector version registered for governed review.",parserVersion:next.parserVersion,implementationHash:next.implementationHash};
...
export function decideOfficialEvidenceConnector(input:{sourceId:OfficialEvidenceSourceId;decision:"APPROVE"|"SUSPEND";reviewerId:string;reviewerName:string;reason:string;decidedAt?:string}): OfficialEvidenceConnectorRegistration {
  ...
  const state=readDurableConnectorRegistry(); const receipt:ConnectorReviewReceipt={receiptId:randomUUID(),connectorId:next.connectorId,sourceId:next.sourceId,decision:input.decision,actorId:input.reviewerId,actorName:input.reviewerName,decidedAt,reason:input.reason,parserVersion:next.parserVersion,implementationHash:next.implementationHash};
