...
Draft-export follow-up: the live Seippes PDF exposed a remaining default FSA match narrative and green completion checks around placeholder year rows. Drafts now state that no loan is selected, preserve the rate snapshot date and monthly schedule, and keep revenue reconciliation, debt confirmation, year-model reconciliation and financing selection open. TBD/pending or mismatched-length year rows cannot pass the year-model gate. This is a draft completeness control, not certification of all legacy report gates.
