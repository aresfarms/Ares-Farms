...
export function FarmAgricultureTab({ bestUse }: { bestUse: FarmBestUse | null }) {
  ...
  const rankingSupported = bestUse.evidenceStatus === "supported-screen";
  return (
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 10 }}>
        {bestUse.options.map((option, index) => {
          ...
          return (
            <article key={option.name} style={{ ...card, borderColor: rankingSupported && index === 0 ? "#1C4532" : "#E5E0D5", display: "grid", gap: 7, alignContent: "start" }}>
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                ...
                <strong style={{ color: "#1C2B45", fontSize: 14.5 }}>{rankingSupported ? `${index + 1}. ` : ""}{option.name}</strong>
              </div>
