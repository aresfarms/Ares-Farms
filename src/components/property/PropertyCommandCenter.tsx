    ...
    <section aria-label="Property command center" data-testid="property-command-center" data-consumer-lane={lane} style={shell}>
      ...
      <div style={{ padding: 20, display: "grid", gap: 16 }}>
        ...
        {tab === "financing" && <>
          ...
          <section style={card}><h3 style={{ margin: 0, color: "#1C2B45", fontSize: 16 }}>Potential financing pathways</h3><div style={{ display: "grid", gap: 9, marginTop: 10 }}>{props.financingLanes.length ? props.financingLanes.map((item, index) => <div key={item} style={{ border: `1px solid ${index === 0 ? "#B08A2E" : "#E5E0D5"}`, borderRadius: 10, padding: "11px 13px", background: index === 0 ? "#FBF5E6" : "#fff" }}><strong style={{ color: "#1C2B45" }}>{item}</strong><span style={{ display: "block", fontSize: 11.5, color: "#5A6172" }}>{index === 0 ? "Relevant preliminary pathway from the available property evidence. This is not borrower qualification." : "Relevant pathway to examine; price, appraisal, condition, occupancy, and borrower eligibility still control fit."}</span></div>) : <span style={{ color: "#5A6172" }}>No verified borrower-specific match exists yet. Property-relevant financing families will still appear here when the asset type supports them.</span>}</div></section>
