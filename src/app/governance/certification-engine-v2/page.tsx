...
type ApiResponse = {
  ok: boolean;
  error?: string;
  v2Result?: CertificationEngineV2Result;
  governance?: {
    traceId?: string;
...
function statusToTone(
  status: CertificationEngineV2Dimension["status"]
...
function DimensionCard(props: {
  dimension: CertificationEngineV2Dimension;
}) {
      ...
      >
        <div>
          ...
          <div style={{ ...mutedText, fontSize: 12 }}>
            coverage {dimension.coverageCount} · evidence entries{" "}
            {dimension.evidenceEntryCount} · readiness{" "}
...
function ConflictCard(props: {
  conflict: CertificationEngineV2CrossSourceConflict;
}) {
  return (
    <div
      style={{
    ...
    >
      ...
      <div style={{ ...mutedText, fontSize: 11, marginTop: 4 }}>
        Resolution: {props.conflict.resolution} · Route:{" "}
        {props.conflict.reviewRoute}
      </div>
    </div>
  );
}

export default function CertificationEngineV2Page() {
  ...
  const localInput = useMemo<CertificationEngineV2Input>(
    () => ({
      reviewerRole,
      borrowerContext: {
  ...
  const previewResult = useMemo(
    () => composeCertificationEngineV2(localInput),
    [localInput]
  );
  ...
  async function runComposition() {
    ...
    try {
      const response = await fetch(
        "/api/governance/certification-engine-v2",
        {
          method: "POST",
  ...
  return (
    <div style={shellStyle}>
      <div style={containerStyle}>
          ...
          >
            <div>
              <div style={{ fontWeight: 700, fontSize: 20 }}>
                Certification Engine v2
              </div>
              <div style={{ ...mutedText, marginTop: 4 }}>
                Internal certification posture composition over Evidence
                Engine v2 (and the full canonical v2 stack) plus the legacy
                v1 internal certification engine. Internal certification
                only — no external certification, public verification,
                regulatory reliance, lender commitment, or any autonomous
                determination is produced; no live external fetch; no
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              ...
              <StatusBadge tone="neutral" label="Internal certification only" />
            </div>
          </div>
        </header>
        ...
        <section style={{ ...panelStyle, padding: 18 }}>
          ...
          >
            {[
              ...
              ["v1 certified", result.summary.v1CertifiedCount],
        ...
        <section style={{ ...panelStyle, padding: 18 }}>
          <div style={{ fontWeight: 600, fontSize: 16, marginBottom: 12 }}>
            v2 governed certification dimensions
          </div>
        ...
        <section style={{ ...panelStyle, padding: 18 }}>
          <div style={{ fontWeight: 600, fontSize: 16, marginBottom: 12 }}>
            Legacy v1 certification domains ({result.legacyDomains.length})
          </div>
          {result.legacyDomains.map((domain) => (
            ...
            >
              ...
              <div style={{ ...mutedText, fontSize: 11 }}>
                status {domain.status} · readiness {domain.readinessPercent}% ·
