...
export function AgriculturalProFormaPanel(props: { acreage: number; price: number; rate: number; theme: ChartTheme }) {
  ...
  const calculation = useMemo(() => { try { return { model: buildAgriculturalProForma(inputs), error: null }; } catch (error) { return { model: null, error: error instanceof Error ? error.message : "Check the scenario inputs." }; } }, [inputs]);
  ...
  if (!model) return <section role="alert"><p>{calculation.error}</p><button type="button" onClick={() => setInputs(defaultAgriculturalProFormaInputs({ tractAcres: props.acreage, purchasePrice: props.price, annualRatePct: props.rate }))}>Reset scenario inputs</button></section>;
  ...
  return <section style={{ display: "grid", gap: 12, padding: 16, border: `2px solid ${props.theme.accent}`, borderRadius: 12, background: props.theme.plate }}>
    <div><strong style={{ display: "block", fontSize: 16, color: props.theme.ink }}>Working-farm what-if pro forma</strong><span style={{ fontSize: 12, color: props.theme.inkSoft }}>Editable tract-level underwriting model. Assumptions are visible and do not masquerade as verified farm records.</span></div>
    ...
    <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}><tbody>
      ...
      <tr><th style={{ textAlign: "left", padding: 7 }}>Cash-rent alternative</th><td style={{ padding: 7 }}>{money(model.revenue.cashRentLow)}–{money(model.revenue.cashRentHigh)}/yr · {model.debt.cashRentDscrLow == null || model.debt.cashRentDscrHigh == null ? "Lease DSCR pending owner-paid costs; this is gross rent, not NOI." : `DSCR ${model.debt.cashRentDscrLow.toFixed(2)}x–${model.debt.cashRentDscrHigh.toFixed(2)}x`}</td></tr>
    </tbody></table></div>
