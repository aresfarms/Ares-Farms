...
export default function FinancialConnectClient({
  ...
  const onSuccess = useCallback(
    async (publicToken: string | null) => {
      ...
      if (!response.ok || !data?.ok) {
        setStatus(
          data?.error || "The account connection could not be secured.",
        );
        setBusy(false);
        return;
      }
      setStatus(
        syntheticFixture
          ? `Connected synthetic run ${syntheticFixture.testRunId}. The Plaid token is encrypted and lineage-bound.`
          : "Connected. Your Plaid access token is stored only in Furlong's encrypted vault.",
      );
      setBusy(false);
    },
  ...
  const { open, ready } = usePlaidLink({
    ...
    onExit: (error) => {
      if (error)
        setStatus("Plaid Link closed before the connection completed.");
    },
  });
  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "32px 24px 64px" }}>
      {syntheticFixture ? (
        ...
        >
          <strong>
            SYNTHETIC PLAID TEST — {syntheticFixture.humanVisibleName}
          </strong>
          <code>{syntheticFixture.syntheticPersonaId}</code>
          <code>{syntheticFixture.testRunId}</code>
          <span>
            {syntheticFixture.fixtureVersion} · {syntheticFixture.environment} ·{" "}
            {syntheticFixture.operatorIdentity} · {syntheticFixture.createdAt}
          </span>
        </section>
      ) : null}
      <h1>Connect financial accounts</h1>
      <p>
        Furlong requires a fresh passkey verification and a separate
        financial-account authorization before Plaid Link can open.
      </p>
      <div
        style={{
      ...
      >
        <label style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
          ...
          />
          <span>
            <strong>Plaid financial-account authorization</strong>
            <br />
            {consent.text}
          </span>
        </label>
      ...
      <p aria-live="polite" style={{ marginTop: 18 }}>
        {status}
      </p>
      <p style={{ marginTop: 28, fontSize: 14 }}>
        Furlong does not receive your online-banking password. Long-lived Plaid
        access tokens are encrypted before persistence and are subject to
        disconnect, retention, and deletion controls.
      </p>
    </div>
  );
}
