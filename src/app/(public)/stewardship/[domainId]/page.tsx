...
export default async function StewardDomainPage({
  ...
  return (
    <div style={container}>
        ...
        {/* Domain header
            Governance: Operational role-holder names are NOT displayed on public
            surfaces (Operational Roster Clause, Volume VII Annex). Show domain
            titles and guiding-beacon copy only. */}
        <header style={{ display: "grid", gap: 8, marginTop: -12 }}>
          <h1 style={{ margin: 0, fontSize: 32, lineHeight: 1.15 }}>
            {domain.domainName}
          </h1>
          <p style={{ ...muted, margin: 0, fontSize: 15, fontWeight: 600 }}>
            {domain.stewardTitle}
          </p>
        ...
        {/* Page-level footer: contextual back-link only. Site nav is in the layout header/footer. */}
        <div>
          <Link
            href="/stewardship"
            style={{ color: "#0f766e", fontSize: 14, textDecoration: "none" }}
